...
def create_value_type(db: Session, value_type: ValueTypeCreate):
    ...
    print ("Fase 5 de la funcion de creacion \n")
...
def get_value_type_by_name(db: Session, name: str):
    db_value_type = db.query(value_type_model.ValueType).filter(value_type_model.ValueType.name == name).first()

    if not db_value_type:

        try:

            print("\n********Antes de crearlo*******\n")

            new_value_type= create_value_type(db=db,value_type=ValueTypeCreate(
                name=name
            ))
